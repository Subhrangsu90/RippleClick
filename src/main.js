...
function registerAppShortcuts() {
	...
	for (const registration of registrations) {
		try {
			const ok = globalShortcut.register(registration.shortcut, registration.run);
			if (!ok && registration.shortcut !== registration.fallback) {
				globalShortcut.register(registration.fallback, registration.run);
			}
		} catch (error) {
			console.error(`shortcut registration failed: ${registration.shortcut}`, error);
			globalShortcut.register(registration.fallback, registration.run);
		}
	}
...
function getRunnableHookScriptPath() {
	if (!app.isPackaged) return hookScript;

	const targetPath = path.join(app.getPath("userData"), "mouse-hook.ps1");
	const source = fs.readFileSync(hookScript);

	try {
		const current = fs.readFileSync(targetPath);
		if (Buffer.compare(source, current) === 0) return targetPath;
	} catch {
		// The helper has not been extracted yet, or it needs to be refreshed.
	}
...
function startMouseHook() {
	...
	let runnableHookScript;
	try {
		runnableHookScript = getRunnableHookScriptPath();
	} catch (error) {
		console.error("mouse hook script could not be prepared:", error);
		return;
	}

	hookProcess = spawn(
		"powershell.exe",
		["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", runnableHookScript],
		{
			windowsHide: true,
